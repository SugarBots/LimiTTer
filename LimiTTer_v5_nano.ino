/* LimiTTer sketch for the Arduino Nano.
   ...
   Wiring for nano :
    ...
    Arduino          BM019          HM-11
  IRQ: Pin 9       DIN: pin 9        VCC: 3V3
  SS: pin 10       SS: pin  10       RX: 5
  MOSI: pin 11     MOSI: pin 11      TX: 4
  MISO: pin 12     MISO: pin 12
  SCK: pin 13      SCK: pin 13
  I/O :pins 6,7,8  Power: 8,7,6
*/
...
const int NFCPin3 = 6; // Power pin BM019
const int BLEPin = 3; // BLE power pin. Disable this for Android 4
const int MOSIPin = 11; //11;
const int SCKPin = 13; //13;
const int MISOPin = 12; //13;
byte RXBuffer[24];
byte NFCReady = 0;  // used to track NFC state
byte FirstRun = 1;
int sleepTime = 36;//  28; // SleepTime. Set this to 36 for Android 4
float lastGlucose;
float trend[16];

SoftwareSerial ble_Serial(5, 4); // RX | TX
...
void setup() {
  ...
  long bleBaudrate[9] = {1200, 2400, 4800, 9600, 19200, 38400, 57600, 115200, 230400};
  for (int i = 0; i < 9; i++)
  {
    ...
    if (c == 'O' && d == 'K')
      break;
     Serial.println(bleBaudrate[i]);
      
  }
  delay(100);
  ble_Serial.write("AT+ADDR?");
  delay(500);
   String tempString = "";
   for (int i = 0; i < 12; i++)
  {
    char e = ble_Serial.read();
    if(i>=7){
    tempString += (e);}
   }

  tempString = "LimiTTer" + tempString;
  Serial.println(tempString);
  ble_Serial.print("AT+NAME" + tempString);
  delay(500);
  //ble_Serial.write("AT+PASS0000");
